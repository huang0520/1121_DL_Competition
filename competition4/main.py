...
class FunkSVDRecommender(tf.keras.Model):
    ...
    def eval_predict_onestep(self, query: tf.Tensor) -> tf.Tensor:
        ...
        y_recommends = tf.math.top_k(y_pred, k=SLATE_SIZE).indices

        return y_recommends

    @tf.function
    def eval_update_onestep(self, uids: tf.Tensor, histories: tf.Tensor, ratings: tf.Tensor) -> None:
...
print(type(uids))
print(histories)
# train_data = tf.data.Dataset.from_tensor_slices((uids, histories, ratings))
# train_data[0]

#%%

model = FunkSVDRecommender(m_users=N_TEST_USERS, n_items=N_ITEMS, embedding_size=EMBEDDING_SIZE, learning_rate=LEARNING_RATE)
...
for _ in range(TEST_EPISODES):
    ...
    # model = tf.keras.models.load_model('model_funk_svd')
